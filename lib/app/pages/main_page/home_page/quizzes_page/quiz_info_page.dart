...
class _QuizInfoPageState extends State<QuizInfoPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      if ((ModalRoute.of(context)!.settings.arguments as List)[0]
          is QuizModel) {
                              ...
                              children: [
                                ...
                                Text(
                                  status == 'waiting'
                                      ...
                                      : status == 'passed'
                                          ? beforTabName == 'StudentResults'
                                              ? appText.studentPassedTheQuiz
                                              : appText.youPassedTheQuiz
  ...
  Widget pageButton() {
    ...
    if (isTeacher() && beforTabName == 'StudentResults') {
      if (status == 'waiting' && (quizAllData?.reviewable ?? false)) {
        child = button(
            onTap: () {
              nextRoute(QuizPage.pageName, arguments: [
      ...
      if (quizData?.authstatus != 'passed' && beforTabName == 'MyResults') {
        child = button(
            onTap: () async {
              bool? res = await nextRoute(QuizPage.pageName,
                  arguments: [Quiz.fromJson(quizData!.toJson())]);
    ...
    if (child == null) {
      return const Positioned(bottom: 0, right: 0, left: 0, child: SizedBox());
    }
