                      ...
                      child: button(
                          onTap: () async {
                            ...
                            downloadSheet(data!.link!,
                                '${data!.date ?? "certificates$epochTimeString".replaceAll(' ', '_')}.pdf',
                                isOpen: true, isPDf: true,
                                onCertificateId: (String certificateId) {
                              setState(() {
                                if (certificateId != null) {
                                  data?.certificate?.id =
                                      int.parse(certificateId);
                                }
                              });
                            });
                          },
                      ...
                      child: button(
                          onTap: () async {
                            ...
                            if (await DownloadManager.findFile(
                                directory, '${data!.date}.pdf',
                                isOpen: false)) {
                              Share.shareXFiles(
                                  [XFile('$directory/${data!.date}.pdf')]);
                            } else {
                              int epochTime =
                                  DateTime.now().millisecondsSinceEpoch ~/ 1000;
                              String epochTimeString = epochTime.toString();
                              String? res = await downloadSheet(data!.link!,
                                  '${data!.date ?? "certificates$epochTimeString".replaceAll(' ', '_')}.pdf',
                                  isOpen: false, isPDf: true,
                                  onCertificateId: (String certificateId) {
                                setState(() {
                                  if (certificateId != null) {
                                    data?.certificate?.id =
