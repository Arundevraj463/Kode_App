...
class SettingWidget {
  static Widget generalPage(
    ...
    languageController.text = locator<AppLanguage>()
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          input(emailController, emailNode, appText.email,
              isBorder: true, title: appText.email),

          space(14),

          input(nameController, nameNode, appText.name,
              isBorder: true, title: appText.name),

          space(14),

          input(phoneController, phoneNode, appText.phone,
              isBorder: true, title: appText.phone),
        ...
        children: [
          Padding(
            ...
            child: Column(
              ...
              children: [
                space(12),
                input(currentPasswordController, currentPasswordNode, '',
                    isBorder: true, title: appText.currentPassword),
                space(14),
                input(newPasswordController, newPasswordNode, '',
                    isBorder: true, title: appText.newPassword),
                space(14),
                input(retypePasswordController, retypePasswordNode, '',
                    isBorder: true, title: appText.retypePassword),
                space(18),
                Text(
          ...
          SingleChildScrollView(
            ...
            child: DataTable(
              headingRowColor:
                  MaterialStateProperty.all(greyE7.withOpacity(.2)),
              ...
              columns: [
                DataColumn(
                    label: Text(appText.os,
                        style: style14Bold().copyWith(color: Colors.black))),
                DataColumn(
                    label: Text(appText.browser,
                        style: style14Bold().copyWith(color: Colors.black))),
                DataColumn(
                    label: Text(appText.device,
                        style: style14Bold().copyWith(color: Colors.black))),
                DataColumn(
                    label: Text(appText.ip,
                        style: style14Bold().copyWith(color: Colors.black))),
              ],
              rows: List.generate(loginHistory.length, (index) {
                return DataRow(
                    selected: false,
                    color: MaterialStateProperty.all((index % 2 == 1)
                        ? greyE7.withOpacity(.2)
                        : Colors.white),
                    cells: [
                      DataCell(Text(loginHistory[index].os ?? '',
                          style: style12Regular().copyWith(color: greyA5))),
                      DataCell(Text(loginHistory[index].browser ?? '',
                          style: style12Regular().copyWith(color: greyA5))),
                      DataCell(Text(loginHistory[index].device ?? '',
                          style: style12Regular().copyWith(color: greyA5))),
                      DataCell(Text(loginHistory[index].ip ?? '',
                          style: style12Regular().copyWith(color: greyA5))),
                    ]);
              }),
            ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          input(accountTypeController, accountTypeNode, appText.accountType,
              isBorder: true,
              title: appText.accountType,
              isReadOnly: true,
              rightIconPath: AppAssets.arrowDownSvg,
          ...
          input(ibanController, ibanNode, appText.iban,
              isBorder: true, title: appText.iban),

          space(14),

          input(accountIdController, accountIdNode, appText.accountID,
              isBorder: true, title: appText.accountID),
          ...
          input(addressController, addressNode, appText.address,
              isBorder: true, title: appText.address),
      ...
      child: StatefulBuilder(builder: (context, state) {
        return Column(
          children: [
            ...
            //       ? null
            //       : LocationService.getProvince(selectedCountry.id!),
            ...
            // // city
            // FutureBuilder(
            //   future: provinceSelectedId == null
            //       ? null
            //       : LocationService.getCity(provinceSelectedId),
            //   builder: (context, snapshot) {
            //     String title = '';
            //
            //     if (snapshot.connectionState == ConnectionState.done) {
            ...
            // // district
            // FutureBuilder(
            //   future: citySelectedId == null
            //       ? null
            //       : LocationService.getDistricts(citySelectedId),
            //   builder: (context, snapshot) {
            //     String title = '';
            //
            //     if (snapshot.connectionState == ConnectionState.done) {
