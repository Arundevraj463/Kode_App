...
class QuizService {
    ...
    if (jsonResponse['success']) {
      for (var j = 0; j < jsonResponse['data']['quizzes'].length; j++) {
        ...
        if (jsonResponse['data']['quizzes_results'] != null) {
          try {
            for (var i = 0;
                i < jsonResponse['data']['quizzes_results'].length;
                i++) {
              if (jsonResponse['data']['quizzes_results'][i]['quiz_id'] ==
                  jsonResponse['data']['quizzes'][j]['id']) {
                counter++;
                userGradeSum += int.tryParse(jsonResponse['data']
                                ['quizzes_results'][i]['user_grade']
                            ?.toString() ??
                        '0') ??
                    0;
              }
  ...
  static Future<List<QuizModel>> getStudentResults() async {
    ...
    try {
      String url =
          '${Constants.baseUrl}panel/quizzes/results/my-student-result';
  ...
  static Future<QuizModel?> reviewQuiz(int quizId, String beforTab) async {
    try {
      String url = beforTab == 'StudentResults'
          ? '${Constants.baseUrl}panel/quizzes/results/$quizId'
          : '${Constants.baseUrl}panel/quizzes/$quizId/result';
  ...
  static Future<bool> storeResult(
    ...
    try {
      ...
      Response res = await httpPostWithToken(
          url, {"quiz_result_id": quizResultId, "answer_sheet": data});
  ...
  static Future<bool> storeReview(
      int quizResultId, List<Question> questions) async {
    try {
      String url =
          '${Constants.baseUrl}panel/quizzes/results/$quizResultId/review';
      ...
      for (var i = 0; i < qId.length; i++) {
        body.addEntries({
          'value$i': {"question_id": qId[i], "answer": aId[i], "grade": gId[i]}
        }.entries);
      }
